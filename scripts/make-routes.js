/**
 * 공항 자료를 data/airports.json 에서 읽어 브라우저가 쓸 src/routedata.js 로 옮긴다.
 *
 * 앱은 묶음 도구 없이 <script> 만으로 돌아가고, 아티팩트 판은 파일 하나로 합쳐지므로
 * 실행 중에 JSON 을 받아 올 수가 없다. 그래서 자료는 JSON 에 두고 이 스크립트가
 * 그대로 옮겨 적는다. 공항 이름을 더할 때는 data/airports.json 만 고치면 된다.
 *
 * 편명->노선 표는 없앴다. 크루넷 원본에 구간이 적혀 있어 짐작할 까닭이 없다.
 *
...
`;
  ...
  return { airports: Object.keys(airports).length };
}

if (require.main === module) {
  const n = build();
  console.log(`src/routedata.js: 공항 ${n.airports}곳`);
}

module.exports = { build, AIRPORTS, OUT };
